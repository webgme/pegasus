...
define([
    ...
    PegasusPlugin.prototype.main = function (callback) {
        ...
        if (!self.isMetaTypeOf(self.activeNode, self.META.Macro))
            self._errorMessages(self.activeNode,
                "Current project is an invalid type. Please run the plugin on a macro.");
    ...
    PegasusPlugin.prototype._getChildrenAndClearPreview = function() {
        ...
        for(i=0;i<childrenPaths.length;i++) {
            if (self.isMetaTypeOf(self._nodeCache[childrenPaths[i]], self.META.InPreviewAspect)) {
    ...
    PegasusPlugin.prototype._createGraph = function(nIds) {
        ...
        while (nIds.length) {
            ...
            if (!self.isMetaTypeOf(n, self.META.Connection)) {
                ...
                if (self.isMetaTypeOf(guid, self.META.Job)) {
        ...
        for(i=0;i<nodes.length;i++) {
            if (nodes[i] !== 'start') {
                n = nodes[i];
                if (self.isMetaTypeOf(n, self.META.FileSet)
                    || self.isMetaTypeOf(n, self.META.File)) {
    ...
    PegasusPlugin.prototype._resolveFileSetsAndGetForks = function() {
        ...
        while (nodeIds.length) {  // BFS
            ...
            if (self.isMetaTypeOf(self.graph[nodeIds[0]].child[0], self.META.Fork)) {
                fork = {
                    start: nodeIds[0],
                    in: [],
                    out: null
            ...
            } else if (self.isMetaTypeOf(nodeIds[0], self.META.Merge)) { //If merge operator
                ...
                assert(self.isMetaTypeOf(self.graph[nodeIds[0]].child[0], self.META.FileSet), "FileSet must follow a Merge operator");
            ...
            } else if ( self.isMetaTypeOf(nodeIds[0], self.META.FileSet) ) {//If the node is a fileset and next is not a fork
    ...
    PegasusPlugin.prototype._copyFork = function(fork) {
        ...
        assert(self.isMetaTypeOf(fork.start, self.META.FileSet), "Fork must start with a fileset");

        //Copy any inside forks
        for (i = fork.in.length; i--;) {
        ...
        self.graph[sfile] = {
            base: self.graph[fork.start].base, 
            child: self.graph[fork.start].child,
        ...
        for(i = 1; i < numCopies; i++) {
            ...
            for(k = 0; k < nodeIds.length; k++) {
                ...
                } else if (self.isMetaTypeOf(nodeIds[k], self.META.File)) {
                    if (i === 1) {
                        params.atr.name += "(2)";
                    } else {
    ...
    PegasusPlugin.prototype._processFileSet = function(fsId) {//return ids: [ first file, ... rest ]
        ...
        while (++i < names.length) {
            ...
            self.graph[id] = {
                base: self.graph[fsId].base,
                child: self.graph[fsId].child,
                baseId: baseId,
                params: params
    ...
    PegasusPlugin.prototype._createFileFromFileSet = function(fsPath, doNotMove) {  // If doNotMove is true, it won't be moved
        var self = this,
            ...
            atr = JSON.parse(JSON.stringify(node.data.atr)),  // This sucks and should be changed
            reg = JSON.parse(JSON.stringify(node.data.reg));

        originalPos = self.core.getMemberRegistry(self.activeNode, "Workspace",
            fsPath, 'position') || self.core.getRegistry(self._nodeCache[fsPath], 'position');
        pos = JSON.parse(JSON.stringify(originalPos));

        atr.name = names[0];

        //Return file to add to the graph
        return {
            baseId: self.META.File, 
            params: {
                atr: atr,
                reg: reg
            },
    ...
    PegasusPlugin.prototype._createConnection = function(srcGuid, dstGuid) {
        var self = this,
            ...
            newConnection = self.core.createNode({
                parent: self.activeNode,
                base: self.META.PreviewConn
            });
    ...
    PegasusPlugin.prototype._createPreviewNode = function(guid) {
        ...
        //Get correct baseId
        if (self.isMetaTypeOf(guid, self.META.File)) {//Create PreviewFile
    ...
    PegasusPlugin.prototype._createDAXFile = function() {
        ...
        while (nodes.length) {
            ...
            if (self.isMetaTypeOf(nodes[0], self.META.Job)) {
                jobs.push(nodes[0]);
            }
    ...
    PegasusPlugin.prototype._saveOutput = function(fileName,stringFileContent,callback) {
        var self = this,
            name = self.projectName.replace(" ", "_")+"_Config",
            artifact = self.blobClient.createArtifact(name);
